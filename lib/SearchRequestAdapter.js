...
var SearchRequestAdapter = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(SearchRequestAdapter, [{
    ...
    value: function _adaptFacetFilters(facetFilters) {
      ...
      var parseFacetFilter = function parseFacetFilter(facetFilter) {
        var facetFilterMatches = facetFilter.match(_this.constructor.FILER_STRING_MATCHING_REGEX);
        console.log(facetFilterMatches);
        var fieldName = "".concat(facetFilterMatches.groups["fieldName"]).concat(facetFilterMatches.groups["notSure"]);
        var fieldValue = "".concat(facetFilterMatches.groups["value"]);
        var negative = false; // Check if is negative facet: https://www.algolia.com/doc/api-reference/api-parameters/facetFilters/
      ...
      var transformedTypesenseFilters = facetFilters.map(function (item) {
        ...
        if (Array.isArray(item)) {
          ...
          var intermediateNegativeFacetFilters = {};
          item.forEach(function (facetFilter) {
            var _parseFacetFilter = parseFacetFilter(facetFilter),
                fieldName = _parseFacetFilter.fieldName,
                fieldValue = _parseFacetFilter.fieldValue,
                negative = _parseFacetFilter.negative; // console.log(fieldName, fieldValue, negative);
          ...
          if (Object.keys(intermediateFacetFilters).length > 1) {
            ...
            console.error("Typesense does not support cross-field ORs at the moment. The adapter could not OR values between these fields: ".concat(Object.keys(intermediateNegativeFacetFilters).join(",")));
          } // Need to transform:
          // intermediateFacetFilters = {
          //     "field1": ["value1", "value2"],
          // }
          ...
          if (Object.keys(intermediateFacetFilters).length > 0) {
            ...
            typesenseFilterString.push("".concat(fieldName, ":=[").concat(fieldValues.join(","), "]")); // console.log(fieldName, fieldValues, typesenseFilterString);
          }

          if (Object.keys(intermediateNegativeFacetFilters).length > 0) {
            var _fieldName = Object.keys(intermediateNegativeFacetFilters)[0];
            var _fieldValues = intermediateNegativeFacetFilters[_fieldName]; // typesenseNegativeFilterString = `${fieldName}:-[${fieldValues.join(",")}]`;

            typesenseFilterString.push("".concat(_fieldName, ":-[").concat(_fieldValues.join(","), "]"));
          }
          ...
          // Into
          //  fieldName:=fieldValue
          var _parseFacetFilter2 = parseFacetFilter(item),
              _fieldName2 = _parseFacetFilter2.fieldName,
              fieldValue = _parseFacetFilter2.fieldValue,
              negative = _parseFacetFilter2.negative;

          var _typesenseFilterString;

          if (negative) {
            _typesenseFilterString = "".concat(_fieldName2, ":- [").concat(fieldValue, "]");
          } else {
            _typesenseFilterString = "".concat(_fieldName2, ":=[").concat(fieldValue, "]");
    ...
    value: function _buildSearchParameters(instantsearchRequest) {
      ...
      var adaptedSortBy = this._adaptSortBy(indexName); // console.log(params);
