...
var TypesenseInstantsearchAdapter = /*#__PURE__*/function () {
  ...
  (0, _createClass2["default"])(TypesenseInstantsearchAdapter, [{
    ...
    key: "_fixFacetCounts",
    value: function _fixFacetCounts(adaptedResponses) {
      var _adaptedResponses = (0, _toArray2["default"])(adaptedResponses),
          primaryResponse = _adaptedResponses[0],
          ancillaryResponses = _adaptedResponses.slice(1);

      console.log(ancillaryResponses.length);
      ancillaryResponses.forEach(function (ancillaryResponse) {
        console.log(ancillaryResponse);
        Object.keys(ancillaryResponse.facets).forEach(function (facetFieldName) {
          Object.keys(ancillaryResponse.facets[facetFieldName]).forEach(function (facetFieldValue) {
            console.log("".concat(facetFieldName, " ").concat(facetFieldValue, " ").concat(ancillaryResponse.facets[facetFieldName][facetFieldValue]));

            if (primaryResponse.facets[facetFieldName][facetFieldValue]) {
              ancillaryResponse.facets[facetFieldName][facetFieldValue] = primaryResponse.facets[facetFieldName][facetFieldValue];
            }
          });
        });
      });
